...
mod tests {
    ...
    fn test_pdf_mean_std_dev() {
        let inputs = [NEG_INFINITY, -3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0, INFINITY];
        let expected = [
            0.0, 0.027, 0.06476, 0.12099, 0.17603, 0.19947, 0.17603, 0.12099, 0.0,
        ];
        for (input, exp) in inputs.iter().zip(expected) {
    ...
    fn test_cdf_mean_std_dev() {
        let inputs = [NEG_INFINITY, -3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0, INFINITY];
        let expected = [
            0.0, 0.02275, 0.06681, 0.15866, 0.30854, 0.5, 0.69146, 0.84134, 1.0,
        ];
        for (input, exp) in inputs.iter().zip(expected) {
    ...
    fn test_ppf_test_data() {
        ...
        assert_in_delta(
            Normal::ppf(0.25, 0.0, 1.0),
            -0.6744897501960817,
            0.0000000000000001,
        );
        assert_in_delta(
            Normal::ppf(0.001, 0.0, 1.0),
            -3.090232306167814,
            0.000000000000001,
        );
        assert_in_delta(
            Normal::ppf(1e-20, 0.0, 1.0),
            -9.262340089798408,
            0.000000000000004,
        );
    }
